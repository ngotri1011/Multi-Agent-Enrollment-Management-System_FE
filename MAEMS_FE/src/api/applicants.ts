...
// Lấy hồ sơ cá nhân của tôi
export async function getMyApplicant(): Promise<CreateApplicantResponse> {
  // api/Applicants/me
  const res =
    await apiClient.get<ApiWrapper<CreateApplicantResponse>>(
      "/api/Applicants/me",
    );
...
export async function uploadApplicantDocuments(
  payload: FormData,
): Promise<Document[]> {
...
//Xem tài liệu đã upload của bản thân
